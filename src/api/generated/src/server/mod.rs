    ...
    C: Send + Sync + 'static,
{
    ...
    Router::new()
        ...
        .route(
            "/sandboxes/{sandbox_id}/rename",
            post(sandboxes_sandbox_id_rename_post::<I, A, E, C>),
        )
        .route(
...
#[derive(validator::Validate)]
#[allow(dead_code)]
struct SandboxesSandboxIdRenamePostBodyValidator<'a> {
    #[validate(nested)]
    body: &'a models::SandboxRenameRequest,
}

#[tracing::instrument(skip_all)]
fn sandboxes_sandbox_id_rename_post_validation(
    path_params: models::SandboxesSandboxIdRenamePostPathParams,
    body: models::SandboxRenameRequest,
) -> std::result::Result<
    (
        models::SandboxesSandboxIdRenamePostPathParams,
        models::SandboxRenameRequest,
    ),
    ValidationErrors,
> {
    path_params.validate()?;
    let b = SandboxesSandboxIdRenamePostBodyValidator { body: &body };
    b.validate()?;
...
#[tracing::instrument(skip_all)]
async fn sandboxes_sandbox_id_rename_post<I, A, E, C>(
    method: Method,
    TypedHeader(host): TypedHeader<Host>,
    cookies: CookieJar,
    headers: HeaderMap,
    Path(path_params): Path<models::SandboxesSandboxIdRenamePostPathParams>,
    State(api_impl): State<I>,
    Json(body): Json<models::SandboxRenameRequest>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
    A: apis::sandboxes::Sandboxes<E, Claims = C>
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || {
        sandboxes_sandbox_id_rename_post_validation(path_params, body)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .sandboxes_sandbox_id_rename_post(&method, &host, &cookies, &claims, &path_params, &body)
        .await;
    ...
    let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::sandboxes::SandboxesSandboxIdRenamePostResponse::Status204_SandboxRenamedSuccessfully
                                                => {
                                                  let mut response = response.status(204);
                                                  response.body(Body::empty())
                                                },
                                                apis::sandboxes::SandboxesSandboxIdRenamePostResponse::Status400_BadRequest
                                                    (body)
                                                => {
                                                  let mut response = response.status(400);
                                                  {
                                                ...
                                                apis::sandboxes::SandboxesSandboxIdRenamePostResponse::Status401_AuthenticationError
                                                    (body)
                                                => {
                                                  let mut response = response.status(401);
                                                  {
                                                ...
                                                apis::sandboxes::SandboxesSandboxIdRenamePostResponse::Status404_NotFound
                                                    (body)
                                                => {
                                                  let mut response = response.status(404);
                                                  {
                                                ...
                                                apis::sandboxes::SandboxesSandboxIdRenamePostResponse::Status409_Conflict
                                                    (body)
                                                => {
                                                  let mut response = response.status(409);
                                                  {
                                                ...
                                                apis::sandboxes::SandboxesSandboxIdRenamePostResponse::Status500_ServerError
                                                    (body)
                                                => {
                                                  let mut response = response.status(500);
                                                  {
