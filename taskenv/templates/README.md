...
Build the tools drive, envd package and deskd package with the commands in the parent guide. Build and publish `Dockerfile.base` to an OCI registry available to the server:
...
Start two instances of the headless base. In an interactive `ubuntu` tmux shell, run `install-dev.sh` on one and `install-desktop.sh` on the other. The desktop script expects the verified upstream `selkies.deb` and built `deskd.deb` under `~/taskenv-install/`. Upload the scripts/packages with `taskenv upload`. Both scripts install from Ubuntu or the project's official repository/installer; Go uses its official archive with a SHA256 check.
...
No GUI is installed into a headless template. Browser and toolchain state stay in the sandbox rootfs and are captured normally. `envd` and `deskd` versions are recorded as installed Debian packages; the tools drive version is separately pinned in every snapshot.
