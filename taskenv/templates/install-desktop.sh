...
sudo apt-get update
sudo env DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends snapd python3-venv python3-tk python3-pil python3-xlib "$work/selkies.deb" "$work/deskd.deb"
...
systemctl --user enable --now deskd-display.service deskd.service
...
for n in {1..30}; do if deskd check; then break; fi; sleep 1; done
deskd check
sudo apt-get clean
rm -rf "$work"
